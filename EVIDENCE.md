...
`logs/cost-log.jsonl`, one line per call.

> ⚠️ **TODO before submission:** the example below is a *failed* call
> (`"success":false"`, all zeros) — it proves failures get logged, not
> that cost tracking works on a successful call. Open
> `logs/cost-log.jsonl`, find a real `"success":true` line, and paste
> that instead. If local Ollama inference genuinely always reports
> `estimatedCostUsd: 0` (likely, since it's free/local), say that
> explicitly rather than leaving an ambiguous example.
...
Reason: animal subject mismatch: expected "red fox", detected "wolf" (caption similarity 0.605 < 0.8)
```
...
```
guard: no_match, reason: No image cleared the similarity floor (best: 0.485 < 0.5)
```
