...
## Phase 4 continued - real database persistence

Added SQLite (better-sqlite3) as real persistence, replacing the JSON files the pipeline was using: images, image_embeddings, posts, suggestions, and reviews tables with indexes and foreign keys (images.category indexed, suggestions.post_id indexed, reviews.decision indexed). Wrote a migration script (src/db/migrate.js) that imports the existing 40 tagged images and their embeddings from the Phase 2/3 JSON output into the database. Updated the API to read image/embedding data from SQLite instead of JSON files, and to persist every match request as a real posts + suggestions row - verified by calling /posts/match twice and confirming GET /suggestions returns both rows, most recent first, not just the immediate response. Chose SQLite over a full Postgres/Docker setup given the corpus size (40 images) and time constraints - the brief explicitly allows this ('in-DB arrays fine at this scale'), and it satisfies the real requirement (indexed relational tables, not flat files) without the setup overhead.
